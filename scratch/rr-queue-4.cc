...
int main(int argc, char* argv[])
{
	...
	std::ofstream outFile("simulation_results_RR_4.csv", std::ios::out);
	if (!outFile.is_open()) {
    	std::cerr << "Error opening output file!" << std::endl;
    	return 1;
	}
	// Redirect std::cout sang file pcrq_probabilities.txt (sau khi kiểm tra outFile)
	std::ofstream pcrqLog("pcrq_probabilities.txt");
	std::streambuf* oldCout = std::cout.rdbuf(pcrqLog.rdbuf());

	// Write CSV header
	outFile << "n_packets,FlowID,SourceAddress,DestinationAddress,TxBytes,RxBytes,TxPackets,RxPackets,start time, stop time, duration, Throughput_Mbps,OfferedLoad_Mbps,QueueLen_pkts,MeanDelay_ms\n";
    ...
    for (uint32_t n = 1; n < c.GetN(); ++n) {
    	...
    	// Enable tracing
    	wifiPhy.EnablePcap("rr_trace_4", devices);
    	if (tracing) {
        	AsciiTraceHelper ascii;
        	wifiPhy.EnableAsciiAll(ascii.CreateFileStream("rr_q_4" + std::to_string(n_packets) + ".tr"));
    	}
    	...
    	// ==========================================
    	// PCRQ probability output is printed directly from wifi-mac-queue.cc via std::cout.
    	// No Config::Connect needed since we embed the logging directly in the core.
    	// ==========================================
